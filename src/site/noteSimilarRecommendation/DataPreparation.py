...
class CorpusProcess(object):
    def __init__(self):
        """初始化"""
        self.pos_seq = None
        self.train_corpus_path = dir + "resultD.txt"
        self.process_corpus_path = dir + "result-rmb.txt"
        self._maps = {u'ns':u'LOC',u'n': 'F'}
    ...
    def pre_process(self):
        """语料预处理 """
        lines = read_corpus_from_file(self.train_corpus_path)
        new_lines = []
        for line in lines:
    ...
    def initialize(self):
        """初始化 """
        lines = read_corpus_from_file(self.process_corpus_path)
    ...
    def generator(self):
        """训练数据"""
        word_grams = [segment_by_window(word_list) for word_list in self.word_seq]
...
if __name__ == '__main__':
    ner = CRF_NER()
    model = ner.train()
    file_path4 = r'D:\肖红娇\项目\爱旅游网站\原始数据\游记数据0.xlsx'
    raw_data= pd.read_excel(file_path4,header=0)
    dataT4 = raw_data.iloc[1:10800].values

    df=pd.read_excel(r'D:\肖红娇\项目\爱旅游网站\原始数据\总游记数据.xlsx')

    # dictT = []
    List = []
    dataC = []
    for j in range(len(dataT4)):
        dataI = ner.predict(dataT4[j][0]).split(' ')
        # print(dataI)
        for i in dataI[::-1]:
            if len(i)<2:
                dataI.remove(i)
            else:
                dataC.append(re.sub(u'([^\u4e00-\u9fa5])','',i))
        if len(dataC)==0:
            dataC=df['trip'][j]
            # dataC.append('游玩')
            # print(df['trip'][j])
    ...
    with open('data/train.txt', 'w',encoding='UTF-8') as outf:
        for item in List:
            for word in item:
                outf.write(word+' ')
                outf.write('\n')
